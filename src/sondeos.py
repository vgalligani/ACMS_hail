...
#------------------------------------------------------------------------------
def generate_random_latlon(center_lat, center_lon, num_points=3, max_offset_km=30):
    """
...
def plot_sondeos_compare(random_points, title, server, sndLoc):
    ...
    for i in range(len(random_points)):
        ...
        skew.plot(np.nan,np.nan,'cyan',linewidth=2, label='P3(WSM6init)')
