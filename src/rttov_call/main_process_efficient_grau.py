...
# nohup python main_process_efficient_grau.py > output.log 2>&1 &
if __name__ == "__main__":
    ...
    exp     = sys.argv[1]     # or float() if needed
    isnow   = int(sys.argv[2])     # or float() if needed
    
    # rttov iwcrescaling: 
    # allskyfile = f'WSM6_rsg_sliu{isnow}grausp'+exp            
    # expname    = 'allsky_liusnow'+str(isnow)+'_grausp'+exp
    # main_Process('MHS', '20:30', 6.1, 'yakaira', allskyfile, expname, 'iwcrescaling')
    # print('Finished allsky for isnow: ', str(isnow) )  
    
    # rttov eqmass: 
    allskyfile = f'eqMass_WSM6_rsg__sliu{isnow}grausp'+exp            
    expname    = 'allsky_liusnow'+str(isnow)+'_grausp'+exp
    main_Process('MHS', '20:30', 6.1, 'yakaira', allskyfile, expname, 'eqmass')
    print('Finished allsky for isnow: ', str(isnow) )  
        
    # sieron: 
    # allskyfile = f'sliu{isnow}_grausp_aggregatesliu{isnow}dummy_2snowprofile_cirsgs_6nhydro_'+exp            
    # expname    = 'allsky_liusnow'+str(isnow)+'_grausp_aggregatesliu'+str(isnow)+exp
